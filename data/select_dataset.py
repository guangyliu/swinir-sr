...
def define_Dataset(opt, train_files, rotation, flip):
    dataset_type = opt['dataset_type'].lower()
    ...
    # super-resolution
    # -----------------------------------------
    elif dataset_type in ['python_cufed']:
        from data.PIL_bicubic_downsample import ReferenceDataset as D

    elif dataset_type in ['matlab_cufed']:
        from data.dataset_reference import ReferenceDataset as D
    ...
    dataset = D(opt, train_files, rotation, flip)
    print('Dataset [{:s} - {:s}] is created.'.format(dataset.__class__.__name__, opt['name']))
    return dataset
