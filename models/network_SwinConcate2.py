...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        self.ape = ape  # False: do not add absolute position embedding to the patch embedding
        self.patch_norm = patch_norm  # True: add normalization after patch embedding
        self.num_features = embed_dim  # 96
        ...
        if self.ape:
            self.absolute_pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim * 2))
            trunc_normal_(self.absolute_pos_embed, std=.02)

        self.image_embeds_1 = nn.Parameter(torch.randn(1, embed_dim, 1, 1))  # first image embedding
        trunc_normal_(self.image_embeds_1, std=.02)
        self.image_embeds_2 = nn.Parameter(torch.randn(1, embed_dim, 1, 1))  # second image embedding
        trunc_normal_(self.image_embeds_2, std=.02)
        ...
        # print('patches_resolution:', patches_resolution) #[264, 184]
        ...
        self.norm = norm_layer(self.num_features*2)  # 96

        # build the last conv layer in deep feature extraction
        if resi_connection == '1conv':
            self.conv_after_body = nn.Conv2d(embed_dim * 2, embed_dim * 2, 3, 1, 1)
        elif resi_connection == '3conv':
            # to save parameters and memory
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            self.conv_before_upsample = nn.Sequential(nn.Conv2d(embed_dim * 2, embed_dim, 3, 1, 1),
                                                      nn.LeakyReLU(inplace=True),
                                                      nn.Conv2d(embed_dim, num_feat, 3, 1, 1),
                                                      nn.LeakyReLU(inplace=True)
                                                      )  # 96 --> 64
            self.upsample = Upsample(upscale, num_feat)
            self.conv_last = nn.Sequential(nn.Conv2d(num_feat, num_out_ch, 3, 1, 1))  # 96 --> 3
    ...
    def forward_features(self, x):
        ...
        x = self.patch_embed(x)  # torch.Size([1, 60, 264, 184]) -> torch.Size([1, 48576, 60])
        if self.ape:
    ...
    def forward(self, ref, lr_up):
        self.mean = self.mean.type_as(ref)
        ref = (ref - self.mean) * self.img_range
        lr_up = (lr_up - self.mean) * self.img_range  # 1

        if self.upsampler == 'pixelshuffle':
            # for classical SR
            lr_up = self.conv_first(lr_up)
            ref = self.conv_first(ref)  # torch.Size([10, 180, 160, 160])
            lr_up += self.image_embeds_1  # Add image embedding torch.Size([1, 180, 1, 1]) add a vector
            ref += self.image_embeds_2
            x = torch.cat([lr_up, ref], 1)  # dim*2
            x = self.conv_after_body(self.forward_features(x)) + x  # torch.Size([1, 60, 264, 184])
            x = self.conv_before_upsample(x)  # torch.Size([1, 64, 264, 184])
            # x = self.conv_last(self.upsample(x))  # torch.Size([1, 3, 1056, 736])
            x = self.conv_last(x)  # torch.Size([1, 3, 1056, 736]) # No need to upsample
