...
class SwinRefSR(nn.Module):
    ...
    def __init__(self, img_size=160, patch_size=1, in_chans=3,
        ...
        for i_layer in range(self.backbone_num_layers):  # how many layers of RSTB 4
            layer = RSTB(dim=embed_dim,
                         input_resolution=(patches_resolution_small[0],
                                           patches_resolution_small[1]),
                         depth=backbone_depths[i_layer],  # depths: [3,3,3,3]
                         num_heads=backbone_num_heads[i_layer],  # num_heads: [3,3,3,3]
                         window_size=window_size,
    ...
    # def ref_forward_features(self, lr, ref_down, ref):
    #     '''
    #     input: lr_up, ref_down_up, ref
    #     '''
    #     small_size = (lr.shape[2], lr.shape[3])
    #     large_size = (ref.shape[2], ref.shape[3])  # [160, 160]
    #
    #
    #     lr = self.patch_embed(lr)  # torch.Size([1, 180, 160, 160]) -> torch.Size([1, 25600, 180])
    #     ref_down = self.patch_embed(ref_down)
    #     ref = self.patch_embed(ref)
    #
    #     if self.ape:
    #         lr += self.absolute_pos_embed_small
    #         ref_down += self.absolute_pos_embed_small
    #         ref += self.absolute_pos_embed
    #     lr_up = self.pos_drop(lr_up)
    #     ref_down_up = self.pos_drop(ref_down_up)
    #     ref = self.pos_drop(ref)
