...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        if self.input_resolution == x_size:
            attn_windows = self.attn(x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C. perform self-attention here to update features
        else:
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        self.norm = norm_layer(self.num_features)  # 96
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_before_upsample = nn.Sequential(nn.Conv2d(embed_dim, num_feat, 3, 1, 1),
                                                      nn.LeakyReLU(inplace=True))  # 96 --> 64
            self.upsample = Upsample(upscale, num_feat)
            self.conv_last = nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)  # 96 --> 3
    ...
    def forward(self, x):
        ...
        x = (x - self.mean) * self.img_range  # 1

        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)  # torch.Size([1, 60, 264, 184])
